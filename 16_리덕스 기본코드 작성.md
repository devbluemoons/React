...
export default connect(
  ...
  {             // 두번째 파라미터: 액션 생성 함수 (객체 형태로 넣어줄 수도 있다
    increase, 
    decreacse
  },
)(ContainerComponent)
...
- 프레젠테이션 컴포넌트 작성
- 컨테이너 컴포넌트에 액션, 액션 생성 함수, 리듀서 코드 작성
- 리듀서가 여러개라면 `루트 리듀서`를 만들어 하나로 합친다
- createStore(reducer)`스토어 생성 함수`에 리듀서를 넣어 스토어를 생성
- <Provider store={store}><App /></Provider> 형태로 프로젝트에 리덕스 적용
- <Container><Presentation /></Container> 프레젠테이션 컴포넌트를 컨테이너 컴포넌트로 감싸준다  
