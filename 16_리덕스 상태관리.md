...
`handleAction`함수를 사용하여 각 액션마다 업데이트 함수를 설정하느 형식으로 작성한다  
  
첫 번째 파라미터에는 각 액션에 대한 업데이트 함수를 넣어 주고, 두 번째 파라미터에는 초기 상태를 넣어준다  
...
const counter = handleAction(
  {
    [INCEREASE]: (state, action) => ({ number: state.number + 1 }),
    [DECEREASE]: (state, action) => ({ number: state.number - 1 }),
  },
  initialState,
);
